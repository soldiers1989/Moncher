...
class OperatorService{
	...
	public function getList($page){
		try{
			...
			$list=$model->getOperatorList($this->session->getSession('Merchant','providerId'),1,($page-1)*20,20);
	...
	public function delete($id){
		try{
			$bool=false;
			$rows=$this->data->delete($this->item,$id==$this->session->getSession('Merchant','id')?0:$id);
			$rows>0 && $bool=true;
			return $bool;
		}catch(Exception $e) {
	...
	public function getTotal(){
		try{
			$model=new OperatorModel();
			$rows=$model->getOperatorList($this->session->getSession('Merchant','providerId'),2);
	 ...
	 ************************************************************
	 */
	public function search($name,$roleId,$areaId,$brandId,$userStatus,$page){
		try{
			$bool=Array();
			$model=new OperatorModel();
			$list=$model->getSearch(1,$this->session->getSession('Merchant','providerId'),$name!=NULL?$name:NULL,$roleId!=NULL?$roleId:NULL,$areaId!=NULL?$areaId:NULL,$brandId!=NULL?$brandId:NULL,$userStatus!=NULL?$userStatus:NULL,($page-1)*20,20);
			(!empty($list) && count($list)>0) && $bool=$list;
			return $bool;
		}catch(Exception $e) {
			$log=new CurrLog(1,3,$this->session->getSession('Merchant','loginName'),$this->system->getSystemTime('A'),'',$e->getMessage());
	 ...
	 ************************************************************
	 */
	public function getSearchTotal($name,$roleId,$areaId,$brandId,$userStatus){
		try{
			$model=new OperatorModel();
			$rows=$model->getSearch(2,$this->session->getSession('Merchant','providerId'),$name!=NULL?$name:NULL,$roleId!=NULL?$roleId:NULL,$areaId!=NULL?$areaId:NULL,$brandId!=NULL?$brandId:NULL,$userStatus!=NULL?$userStatus:NULL);
			return  $rows>0?$rows:0;
		}catch(Exception $e) {
			$log=new CurrLog(1,3,$this->session->getSession('Merchant','loginName'),$this->system->getSystemTime('A'),'',$e->getMessage());
			$log->error();
