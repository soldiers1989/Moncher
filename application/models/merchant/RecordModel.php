...
$selectStr=array(
		...
		$sql.=empty($selectStr[$questionnaireId])?'1 AS  A1':$selectStr[$questionnaireId];
		 $sql.="		FROM   cada_results_questionnaire  				  			  AS A 	
									...
									LEFT JOIN cada_car_info                     		      AS H  ON  H.id=F.brandId
		...
		if((!empty($startTime) && !empty($endTime))){
			strtotime($startTime)<=strtotime($lastDate) ? $sql.=" AND  A.operationTime   BETWEEN  '".$startTime." 00:00:00' AND '".$endTime."  23:59:59'":
		...
		$queri=$this->db->query($sql.' LIMIT '.$open.','.$last);
		return $type==1 ? $queri->result_array():$query->num_rows();	}
	/**
	 ***********************************************************
	 ...
	 ************************************************************
	 */
	public function getResultAreas($type,$questionnaireId,$groupId=0,$brandId=0){
		$sql="SELECT   	 A.areaId,
								 B.title
				FROM       cada_results_questionnaire                        A
								cada_area                 							   B    ON B.id=A.areaId
				WHERE      A.status=1   AND
								 B.status=1   AND
								 A.questionnaireId=".$questionnaireId;
		(!empty($groupId) && $groupId!=0 && $brandId==0)  && $sql.="	AND A.groupId=".$groupId;
		(!empty($brandId) && $brandId!=0 && $groupId==0)  && $sql.="	AND A.brandId=".$brandId;
		$sql.="	GROUP BY A.areaId 	ORDER   BY  A.id DESC";
		$query=$this->db->query($sql);
		return $type==1?$query->result_array():$query->num_rows();
	}
	/**
	 ***********************************************************
	 *方法::RecordModel::getResultBrands
	 * ----------------------------------------------------------
	 * 描述::获取调研中品牌
	 *----------------------------------------------------------
	 *返回：
	 ...
	 ************************************************************
	 */
	public function getResultBrands($type,$questionnaireId,$groupId){
		$sql="SELECT   	 A.brandId,
								 B.name
				FROM       cada_results_questionnaire                        A
				...
				WHERE      A.status=1   AND
								 ...
								 A.questionnaireId=".$questionnaireId;
		(!empty($groupId) && $groupId!=0)  && $sql.="	AND A.groupId=".$groupId;
		$sql.="	GROUP BY A.brandId 	ORDER   BY  A.id DESC";
		$query=$this->db->query($sql);
		return $type==1?$query->result_array():$query->num_rows();
	}
	/**
	 ***********************************************************
	 *方法::RecordModel::getResultProviders
	 * ----------------------------------------------------------
	 * 描述::获取调研中商户列表
	 *----------------------------------------------------------
	 *返回：
	 ...
	 ************************************************************
	 */
	public function getResultProviders($type,$questionnaireId,$groupId){
		$sql="SELECT   	 A.merchantId,
								 B.name
				FROM       cada_results_questionnaire                        A
				...
				WHERE      A.status=1   AND
								 ...
								 A.questionnaireId=".$questionnaireId;
		(!empty($groupId) && $groupId!=0)  && $sql.="	AND A.groupId=".$groupId;
		$sql.="	GROUP BY A.merchantId 	ORDER   BY  A.id DESC";
		$query=$this->db->query($sql);
		return $type==1?$query->result_array():$query->num_rows();
	}
}