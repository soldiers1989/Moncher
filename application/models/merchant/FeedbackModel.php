...
class FeedbackModel extends CI_Model {
	...
	/**
	 ***********************************************************
	 *方法::FeedbackModel::getFeedbackList
	 * ----------------------------------------------------------
	 * 描述::获取列表分页及数量方法
	 *----------------------------------------------------------
	 *参数:
	 ...
	 ************************************************************
	 */
	public function getFeedbackList($type=1,$providerId,$open=0,$last=20,$order="A.id",$desc="DESC"){
		if($type==1){
			...
			$query=$this->db->query("SELECT * FROM  cada_provider_feedback  WHERE status=1 AND  providerId=".$providerId);
	...
	/**商户搜索方法*/
	public function getSearch($type=1,$providerId,$status,$types,$startFeedDate,$lastFeedDate,$startReplyDate,$lastReplyDate,$open=0,$last=20){
		$sql="SELECT 			A.id,
											A.title,
											A.tel,
											A.feedbackTime,
