...
class AreaModel extends CI_Model {
	 ...
	 ************************************************************
	 */
	public function getAreaList($type=1,$Pid=0,$open=0,$last=20,$order='id',$desc='DESC'){
		if($type==1){
			...
			$query=$this->db->query('SELECT * FROM cada_area WHERE status=1 AND parentId='.$Pid);
