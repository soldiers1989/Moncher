...
class OperatorModel extends CI_Model {
	 ...
	 ************************************************************
	 */
	public function getOperatorList($type=1,$open=0,$last=20,$order='id',$desc='DESC'){
		if($type==1){
			$query=$this->db->query("SELECT 			A.*,
																					B.roleName 									 AS  roleName,
																					C.name      										 AS   areaName,
																					D.name	   									 	 AS   sectorName
															 FROM   			cada_admin   									 AS   A
																					LEFT JOIN cada_admin_roles 		 AS B ON B.id=A.staffRoleId 
																					LEFT JOIN cada_area				   		 AS C ON C.id=A.areaId
																					LEFT JOIN cada_dictioncry  			 AS D ON D.id=A.sector
															 WHERE      	A.status=1
															 ORDER  BY    A.".$order."  ".$desc."
															 LIMIT				".$open.",".$last);
			return $query->result_array();
		}else{
			$query=$this->db->query('SELECT * FROM cada_admin WHERE status=1');
	 ...
	 ************************************************************
	 */
	public function getSearch($type=1,$name,$roleId,$areaId,$sector,$userStatus,$open=0,$last=20){
		...
		!empty($sector) && $sql.="			AND    A.sector=".$sector;
