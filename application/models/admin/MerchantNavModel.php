...
class MerchantNavModel extends CI_Model {
	/**
	 ***********************************************************
	 *方法::MerchantNavModel::__construct
	 * ----------------------------------------------------------
	 * 描述::初始化方法
	 *----------------------------------------------------------
	 *参数:
	...
	/**
	 ***********************************************************
	 *方法::MerchantNavModel::getMenuList
	 * ----------------------------------------------------------
	 * 描述::获取列表分页及数量方法
	 *----------------------------------------------------------
	 *参数:
	 ...
	 ************************************************************
	 */
	public function getMerchantNavList($type=1,$Pid=0,$open=0,$last=20,$order='id',$desc='DESC'){
		if($type==1){
			$query=$this->db->query("SELECT 			*
															 FROM   			cada_provider_admin_navs
															 WHERE      	status=1 						AND 
																					pId=".$Pid."		
															 ORDER  BY    ".$order."  ".$desc."
															 LIMIT				".$open.",".$last);
			...
			$query=$this->db->query('SELECT * FROM cada_provider_admin_navs WHERE status=1 AND pId='.$Pid);
